...
public interface QueriesRepository extends CrudRepository<Query, Long> {
	...
	List<Query> findAllByUserOrderByName(User user);
	...
	@org.springframework.data.jpa.repository.Query(value = "select q.* from query q where q.public_for_all = true or q.user_id = ?1 or q.id in (select c.query_id from public_to c where c.user_id = ?1) order by q.name", nativeQuery = true)
	List<Query> findAvailableQueriesForUser(User user);
	
	@org.springframework.data.jpa.repository.Query(value = "select q.* from query q where q.name like ?2 and (q.public_for_all = true or q.user_id = ?1 or q.id in (select c.query_id from public_to c where c.user_id = ?1)) order by q.name limit ?3", nativeQuery = true)
	List<Query> findAvailableQueriesForUserByName(User user, String name, int limit);
